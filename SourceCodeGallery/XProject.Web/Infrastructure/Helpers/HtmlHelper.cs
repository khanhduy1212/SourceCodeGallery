...
namespace XProject.Web.Infrastructure.Helpers
{
    public static class BootstrapHtmlHelper
    {
        ...
        public static IHtmlString RawFixDecodeUTF8(this HtmlHelper htmlHelper, string text,bool metaContent=false)
        {
            if (!metaContent)
            {
                return htmlHelper.Raw(HttpUtility.HtmlDecode(htmlHelper.Raw( text ).ToHtmlString()));
            }

            return htmlHelper.Raw(HttpUtility.HtmlDecode(htmlHelper.Raw("content=\"" + text+"\"").ToHtmlString()));
        }
    }
}
