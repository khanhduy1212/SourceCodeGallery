...
namespace XProject.Web
{
    public static class UrlFriendly
    {
        ...
        public static string UrlFrendlySpace(this string text, int maxlen = 80)
        {
            if (string.IsNullOrEmpty(text)) return "";
            bool prevdash = false;
            var sb = new StringBuilder(text.Length);
        ...
        private static readonly List<string> wordsToRemove = "and or".Split(' ').ToList();
         ...
         public  static  String ExtractParams(String filter, String key)
        {
            String result = "";
            if (!String.IsNullOrWhiteSpace(filter))
            {
                ...
                if (filter.Split('-').Where(T => T == key).Count() > 0)
                {
                    ...
                    if (bString.Length > 0)
                    {
                        ...
                        {
                            if (newFilter.Split('-')[i] == "td")
                            {
                                break; ;
                            }
                            if (newFilter.Split('-')[i] == "tai")
                            {
                                break; ;
                            }
                            if (newFilter.Split('-')[i] == "tuyen_dung")
                            {
                                break; ;
                            }
                            ...
                            {
                                break; ;
                            }if (newFilter.Split('-')[i] == "hv")
                            {
                                break; ;
                            }
                            if (newFilter.Split('-')[i] == "sn")
                            {
                                break; ;
                            }
                            if (newFilter.Split('-')[i] == "kt")
                            {
                                break; ;
                            }
