...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _initPreviewActions: function () {
            ...
            $preview.find(btnRemove).each(function () {
                ...
                vKey = vKey == "" ? config : vKey;
                params = { id: $el.attr('id'), key: vKey, extra: extraData };

                if ($h.isEmpty(self.deleteUrl) || vKey === undefined) {
                    self._handler($el, 'click', function () {
                        var n, cap;
    ...
    $(document).ready(function () {
        ...
        if ($input.length) {
            ...
            $($input).each(function () {
                if (window[$(this).attr('id')] != undefined && window[$(this).attr('id')] != null && window[$(this).attr('id')] != '') {
                    var pluginOptions = JSON.parse(window[$(this).attr('id')]);
                    $(this).fileinput(pluginOptions);
                    $('.file-footer-buttons .kv-file-zoom').removeAttr('disabled');
                } else {
                    $(this).fileinput();
                }
                $(this).on('change', function (event, numFiles, label) {
                    if ($(event.currentTarget).attr('multiple') == null) {
                        if (event.target.value.length > 0) {
                            $('#' + $(event.currentTarget).attr('id').replace('_input', '')).val(event.target.value.substring(event.target.value.lastIndexOf('\\') + 1));
                        }
                    }
                    
                });
                
                $(this).on('beforeChange', function (event, numFiles, label) {
                    if ($(event.currentTarget).attr('multiple') == null) {
                        var $this = $(event.currentTarget);
                         var fileinput = $this.data('fileinput');
                         fileinput.previewCache.data.content = [];
                         fileinput.previewCache.data.config = [];
                    }
                });
                $(this).on('filedeleted',
                    function(event, fileName) {
                        var files = $('#' + $(event.currentTarget).attr('id').replace('_input', '')).val().split(',');
                        var index = files.indexOf(fileName);
                        if (index > -1)
                            files.splice(index, 1);
                        $('#' + $(event.currentTarget).attr('id').replace('_input', '')).val(files.join(','));
                       
                    });
              

                $(this).on('fileclear', function (event) {
                    var $this = $(event.currentTarget);
                    var fileinput = $this.data('fileinput');
                    if ($(event.currentTarget).attr('multiple') == null) {
                        fileinput.previewCache.data.content = [];
                        fileinput.previewCache.data.config = [];
                        $('#' + $(event.currentTarget).attr('id').replace('_input', '')).val("");
                    }
