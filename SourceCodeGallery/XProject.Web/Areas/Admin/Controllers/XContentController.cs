...
namespace XProject.Web.Areas.Admin.Controllers
{
    ...
    public class XContentController : Controller
    {
        //
        // GET: /Admin/BDSNews/
        private readonly IGeneralRepository<XContent> _menuRepository;
        ...
        public XContentController(IGeneralRepository<XContent> menuRepository, IGeneralRepository<XCountry> countryRepository, IGeneralRepository<XType> typeRepository)
        {
        ...
        public ActionResult Create()
        {
            ...
            var model = new XContent();
            model.Order = count + 1;
            return View(model);
        }
        [HttpPost]
        public ActionResult Create(XContent model, HttpPostedFileBase[] files)
        {
            ...
            if (model.XContentsArray != null)
            {

                model.CodeLanguage = model.XContentsArray[0].CodeLanguage;
                model.Name = model.XContentsArray[0].Name;
                model.Description = model.XContentsArray[0].Description;
                model.ShortDescription = model.XContentsArray[0].ShortDescription;
                model.Link = model.XContentsArray[0].Link;
                model.KeySearch = model.XContentsArray[0].Name.NormalizeD() + "\n" + (String.IsNullOrEmpty(model.XContentsArray[0].Description) ? "" : model.XContentsArray[0].Description.NormalizeD()); ;
                ...
                foreach (var item in model.XContentsArray.Skip(1).ToList())
                {
        ...
        public ActionResult Edit(int id)
        {
            ...
            model.XContentsList = _menuRepository.GetIQueryableItems().Where(x => x.CurrentId == model.CurrentId && listCode.Contains(x.CodeLanguage)).ToList();
            return View(model);
        }
        [HttpPost]
        public ActionResult Edit(XContent model, HttpPostedFileBase[] files)
        {
            int id = CurrentUser.Identity.ID;
            ...
            if (model.XContentsArray != null)
            {
                foreach (var item in model.XContentsArray)
                {
