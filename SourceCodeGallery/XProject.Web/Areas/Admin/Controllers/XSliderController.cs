...
namespace XProject.Web.Areas.Admin.Controllers
{
    ...
    public class XSliderController : Controller
    {
        //
        // GET: /Admin/BDSNews/
        private readonly IGeneralRepository<XSlider> _menuRepository;
        ...
        public XSliderController(IGeneralRepository<XSlider> menuRepository, IGeneralRepository<XCountry> countryRepository)
        {
        ...
        public ActionResult Create()
        {
            ...
            var model = new XSlider();
            model.Order = count + 1;
           
            return View(model);
        }
        [HttpPost]
        public ActionResult Create(XSlider model, HttpPostedFileBase[] files)
        {
            ...
            if (model.XSlidersArray != null)
            {

                model.CodeLanguage = model.XSlidersArray[0].CodeLanguage;
                model.Name = model.XSlidersArray[0].Name;
                model.Description = model.XSlidersArray[0].Description;
                model.ShortDescription = model.XSlidersArray[0].ShortDescription;
                model.Link = model.XSlidersArray[0].Link;
                model.KeySearch = model.XSlidersArray[0].Name.NormalizeD() + "\n" + (String.IsNullOrEmpty(model.XSlidersArray[0].Description) ? "" : model.XSlidersArray[0].Description.NormalizeD()); ;
                ...
                foreach (var item in model.XSlidersArray.Skip(1).ToList())
                {
        ...
        public ActionResult Edit(int id)
        {
            ...
            model.XSlidersList = _menuRepository.GetIQueryableItems().Where(x => x.CurrentId == model.CurrentId && listCode.Contains(x.CodeLanguage)).ToList();
            return View(model);
        }
        [HttpPost]
        public ActionResult Edit(XSlider model, HttpPostedFileBase[] files)
        {
            int id = CurrentUser.Identity.ID;
            ...
            if (model.XSlidersArray != null)
            {
                foreach (var item in model.XSlidersArray)
                {
