...
namespace XProject.Web.Areas.Admin.Controllers
{
    ...
    public class XNewController : Controller
    {
        //
        // GET: /Admin/BDSNews/
        private readonly IGeneralRepository<XNew> _menuRepository;
        ...
        public XNewController(IGeneralRepository<XNew> menuRepository, IGeneralRepository<XCountry> countryRepository)
        {
        ...
        [HttpPost]
        public ActionResult Create(XNew model, HttpPostedFileBase[] files)
        {
            ...
            if (model.XNewsArray != null)
            {

                model.CodeLanguage = model.XNewsArray[0].CodeLanguage;
                model.Name = model.XNewsArray[0].Name;
                model.Description = model.XNewsArray[0].Description;
                model.ShortDescription = model.XNewsArray[0].ShortDescription;
                model.Link = model.XNewsArray[0].Link;
                model.KeySearch = model.XNewsArray[0].Name.NormalizeD() + "\n" + (String.IsNullOrEmpty(model.XNewsArray[0].Description) ? "" : model.XNewsArray[0].Description.NormalizeD()); ;
                ...
                foreach (var item in model.XNewsArray.Skip(1).ToList())
                {
        ...
        public ActionResult Edit(int id)
        {
            ...
            model.XNewsList = _menuRepository.GetIQueryableItems().Where(x => x.CurrentId == model.CurrentId && listCode.Contains(x.CodeLanguage)).ToList();
            return View(model);
        }
        [HttpPost]
        public ActionResult Edit(XNew model, HttpPostedFileBase[] files)
        {
            int id = CurrentUser.Identity.ID;
            ...
            if (model.XNewsArray != null)
            {
                foreach (var item in model.XNewsArray)
                {
