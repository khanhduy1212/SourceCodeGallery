...
namespace XProject.Web.Areas.Admin.Controllers
{
    ...
    public class XPartnerController : Controller
    {
        //
        // GET: /Admin/BDSNews/
        private readonly IGeneralRepository<XPartner> _menuRepository;
        ...
        public XPartnerController(IGeneralRepository<XPartner> menuRepository, IGeneralRepository<XCountry> countryRepository)
        {
        ...
        public ActionResult Create()
        {
            ...
            var model = new XPartner();
            model.Order = count + 1;
            return View(model);
        }
        [HttpPost]
        public ActionResult Create(XPartner model, HttpPostedFileBase[] files)
        {
                    ...
                    if (model.GetType().GetProperties().Any(t => t.Name == prop))
                    {
                        if (Request.Files[item] != null && Request.Files[item].ContentLength > 0)
                        {
                            ...
                            var pic = new PictureModel
                            {
                                ...
                                cfile = base64String,
                                typeId = 1,
                                NameTypeUpload = "SizeImagePartner"
            ...
            if (model.XPartnersArray != null)
            {

                model.CodeLanguage = model.XPartnersArray[0].CodeLanguage;
                model.Name = model.XPartnersArray[0].Name;
                model.Description = model.XPartnersArray[0].Description;
                model.ShortDescription = model.XPartnersArray[0].ShortDescription;
                model.Link = model.XPartnersArray[0].Link;
                model.KeySearch = model.XPartnersArray[0].Name.NormalizeD() + "\n" + (String.IsNullOrEmpty(model.XPartnersArray[0].Description) ? "" : model.XPartnersArray[0].Description.NormalizeD()); ;
                ...
                foreach (var item in model.XPartnersArray.Skip(1).ToList())
                {
        ...
        public ActionResult Edit(int id)
        {
            ...
            model.XPartnersList = _menuRepository.GetIQueryableItems().Where(x => x.CurrentId == model.CurrentId && listCode.Contains(x.CodeLanguage)).ToList();
            return View(model);
        }
        [HttpPost]
        public ActionResult Edit(XPartner model, HttpPostedFileBase[] files)
        {
            int id = CurrentUser.Identity.ID;
            ...
            if (model.XPartnersArray != null)
            {
                foreach (var item in model.XPartnersArray)
                {
