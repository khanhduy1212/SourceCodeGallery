...
namespace XProject.Web.Areas.Admin.Controllers
{
    ...
    public class AuditController : Controller
    {
        ...
        public ActionResult Index(string Username = null)
        {
            ...
            records = _tracker.GetAuditRecords().DistinctBy(m => m.Username).ToList().Where(x=>x.TimeAccessed>=date);
        ...
        public ActionResult AjaxGetAudits()
        {
            ...
            {
                var date = DateTime.Now.AddDays(-2);
                audits = _tracker.GetAuditRecords().Where(x => x.TimeAccessed >= date);
            }
            // ReSharper disable once AssignNullToNotNullAttribute
